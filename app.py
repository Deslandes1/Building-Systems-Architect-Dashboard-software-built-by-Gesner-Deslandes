...
# Try to import plotly; show helpful error if missing
try:
    ...
    st.error("⚠️ Plotly is not installed. Please add 'plotly>=5.17.0' to your requirements.txt and redeploy.")
    st.stop()
...
def main_app():
    ...
    with tabs[0]:
        ...
        with c1: st.metric("AHU-1 Supply Temp", f"{random.uniform(18.5, 22.5):.1f} °C", delta="+0.3")
        with c2: st.metric("Chiller Plant Efficiency", f"{random.uniform(0.65, 0.85):.2f} kW/kW", delta="-0.02")
        with c3: st.metric("Space CO₂ (Main Hall)", f"{random.randint(380, 450)} ppm", delta="-12")
        with c4: st.metric("BMS Uptime", "99.98%", delta="+0.01%")
    ...
    with tabs[2]:
        ...
        with col1: st.metric("Main HV Intake", "11 kV", delta="Normal"); st.metric("Transformer 1 Load", "82%", delta="+3%")
        with col2: st.metric("Transformer 2 Load", "76%", delta="-1%"); st.metric("Power Factor", "0.95", delta="+0.01")
        with col3: st.metric("UPS Load (Data Centre)", "43%", delta="-2%"); st.metric("Standby Generator", "Ready", delta="Last test: OK")
    ...
    with tabs[4]:
        ...
        col1, col2 = st.columns(2)
        with col1: st.metric("Annual Energy Consumption", "12.4 MWh", delta="-8% vs 2025"); st.metric("Carbon Intensity", "42 kgCO₂/m²", delta="-11%")
        with col2: st.metric("Renewable Generation (PV)", "345 MWh", delta="+12%"); st.metric("Heat Pump Contribution", "38%", delta="+5%")
